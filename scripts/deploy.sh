...
https_port_value() {
  sd_https_port=${HTTPS_PORT:-$(env_value HTTPS_PORT)}
  sd_https_port=${sd_https_port:-${PORT:-$(env_value PORT)}}
  printf '%s\n' "${sd_https_port:-443}"
}
...
deployment_public_url() {
  sd_public_url_port=$1
  if [ "$use_cloudflare_tunnel" = true ]; then
    sd_public_url_port=443
  fi
  public_url_for "$tls_host" "$sd_public_url_port"
}
...
if [ ! -f "$env_path" ]; then
  ...
  public_url=$(deployment_public_url "$https_port")
  ...
  if [ "$host_was_supplied" = true ]; then
    configured_https_port=$(https_port_value)
    set_env_value PUBLIC_URL "$(deployment_public_url "$configured_https_port")"
  fi
